...
RFCDE <- function(x_train, z_train, lens = rep(1L, ncol(x_train)), #nolint
  ...
  #forest$set_leaves_id() #points of deviance
...
#' @export
predictNew.RFCDE <- function(object, newdata,
                          z_grid = NULL,
                          ...) {
  if (is.vector(newdata)) {
    if (length(newdata) == object$n_x) {
  ...
  terminal_nodes <- rep(0L, n_train)
  for (ii in seq_len(n_test)) {
    terminal_nodes <- weights(object, newdata[ii, , drop = FALSE]) #nolint
  }
...
predict.RFCDE <- function(object, newdata,
  ...
  if (response == "CDE") {
    ...
    #return(quantiles)
  } else if(response == "terminalNode"){
    #forest$rcpp$set_leaves_id() #TODO
    groups <- apply(newdata, 1, function(row){
