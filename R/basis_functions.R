...
evaluate_basis <- function(z, n_basis, system = "cosine") {
  ...
  n_dim <- ncol(z)
  if (n_dim == 1) {
    system <- match.arg(system, c("cosine", "Haar", "Legendre"))
    basis_function <- switch(system,
                             cosine = cosine_basis,
                             Legendre = legendre_basis,
                             Haar = haar_basis)
    
    basis <- basis_function(z, n_basis)
    return(basis)
  } else {
    if (length(n_basis) == 1) {
...
tensor_basis <- function(z, n_basis, systems = "Fourier") {
  ...
  for (ii in 1:prod(n_basis)) {
    id <- ids[ii, ]
    tmp <- lapply(1:n_var, function(nn) { #TODO more elegant?
...
#' Evaluates legendre basis for new observations
#' @inheritParams evaluate_basis
#' @import basefun, variables
#' @return A matrix of dimension length(z) by n_basis with entries
#'   consisting of the first n_basis legendre basis functions evaluated
#'   at the points z
legendre_basis <- function(z, n_basis) {
  ### set-up basis
  variable <- variables::numeric_var("z", support = c(0, 1))
  lb <- basefun::Legendre_basis(variable, order = n_basis-1)
  basis <- lb(z)
  return(basis)
}
