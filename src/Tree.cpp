...
Node Tree::traverse(double* x_test) {
  ...
  while (cur -> split_var != -1) {
    ...
    //pid = reinterpret_cast<std::uintptr_t>(cur); //TODO delete
    //Rcout << pid << ", ";//TODO delete
    Rcout <<"split_var: "<< cur->split_var << ", ";
  }
  //pid = reinterpret_cast<std::uintptr_t>(cur); //TODO delete
  //Rcout << "Tree::traverse after while:" << pid;
  return *cur;
}

int Tree::traverse_idint(double* x_test) {
  //Rcout << "Tree::xtest value" << x_test <<", ";
  ...
  //   x_test: pointer to a new observation
  //
  // Returns: the leaf node pointer id in which x_test ends up.
  Node* cur = &(this -> root);
  Rcout << "Tree::traverse in while:";
  //int pid;
  while (cur -> split_var != -1) {
    if (calculate_feature(x_test, cur -> split_var) <= cur -> split_value) {
      cur = cur -> le_child;
    ...
    //pid = reinterpret_cast<std::uintptr_t>(cur); //TODO delete
    Rcout << cur->split_var << ", ";//TODO delete
  }
  int pid = cur -> leaf_id;
  //pid = reinterpret_cast<std::uintptr_t>(cur); //TODO delete
  ///Rcout << "Tree::traverse after while:" << pid;
