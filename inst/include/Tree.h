...
class Tree {
 public:
  ...
  //template<class INTEGER>
  int traverse_idint(double* x_test);
    //{
    // Node id = traverse(x_test);
    //for(auto it = id.valid_idx_begin; it != id.valid_idx_end; ++it) {
    //  tmp_buf[*it] += 1;
    ...
    //  Node * p = &id;
    // int pid = reinterpret_cast<std::uintptr_t>(p);
    // Rcout <<" returning to traverse_idint "<< pid << ", ";
    // return pid;
    //};
  
  int set_leafids(int current_id = 1, Node* current = NULL) {
    //Rcout << "current_id: " << current_id << ", ";
    if (current == NULL) { 
      current = &(this -> root); 
    }else{
      //Node current_pointer_resolved = *current;
      //Rcout << best_split.var << ", ";
      //this -> split_var = best_split.var;
      //int current_split_var = current_pointer_resolved.split_var;
      //Rcout << "splivar: " << current_split_var << ", ";
    }
    
    if (!current -> is_leaf()) {
      current_id = set_leafids(current_id, current -> le_child);
      current_id = set_leafids(current_id, current -> gt_child);
    }else{
      current->leaf_id = current_id; //TODO only set for leafs
