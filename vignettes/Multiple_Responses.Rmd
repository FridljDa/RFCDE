...
row <- z_grid[1,]
row <- unlist(row)
t<-forest$rcpp$traverse_forest(row)
  
#groups <- apply(z_grid, 1, function(row){
#row <- z_grid[1,]
#row <- unlist(row)
...
```{r}
groups <- predict(forest, c(0.3), "terminalNode", z_grid)
...
```{r}
weights <- predict(forest, c(0.3), "weights", z_grid)
```
